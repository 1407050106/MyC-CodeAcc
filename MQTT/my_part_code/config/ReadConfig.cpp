...
bool ReadConfig::IsCommentChar(char c) {
    switch (c) {
        case '#':
            return true;
        default:
...
bool ReadConfig::AnalyseLine(const std::string &line, std::string &section, std::string &key, std::string &value) {
    if (line.empty())
        return false;
...
bool ReadConfig::ReadConfigInformation(const std::string &filename) {
    settings_.clear();
...
float ReadConfig::ReadFloat(const char *section, const char *item, const float &default_value) {
...
int ReadConfig::ReadInt(const char *section, const char *item, const int &default_value) {
    std::string tmp_s(section);
    std::string tmp_i(item);
    std::map<std::string, std::string> k_v;
    std::map<std::string, std::string>::iterator it_item;
...
std::string ReadConfig::ReadString(const char *section, const char *item, const char *default_value) {
    std::string tmp_s(section);
    std::string tmp_i(item);
